        ...
        for occlusion_active in [True, False]:
            for fov in [0.5*np.pi, np.pi, 1.5*np.pi, 2*np.pi]:
                for i in range(1, 2):
                    ...
                    ServiceSavedModel.saveModel(simulationData=simulationData, path=f"test_singlespeed={use_single_speed}_occl={occlusion_active}_fov={fov}_noiseP={noisePercentage}_d={density}_r={radius}_tmax={tmax}_{i}.json", 
