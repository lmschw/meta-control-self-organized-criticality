...
def initialiseCsvFileHeaders(path, headers=['t', 'i', 'x', 'y', 'u', 'v'], addSwitchValueHeader=True):
    """
...
def createDictList(timestep, positions, orientations, switchValues, switchingActive=True):
    if switchingActive:
        return [{'t': timestep, 'i': i, 'x': positions[i][0], 'y': positions[i][1], 'u': orientations[i][0], 'v': orientations[i][1], 'switchValue': transformSwitchValue(switchValues[i])} for i in range(len(positions))]
    return [{'t': timestep, 'i': i, 'x': positions[i][0], 'y': positions[i][1], 'u': orientations[i][0], 'v': orientations[i][1]} for i in range(len(positions))]

def saveModelTimestep(timestep, positions, orientations, path, switchValues, switchingActive=True):
    dict_list = createDictList(timestep, positions, orientations, switchValues, switchingActive)
...
def loadModelFromCsv(filepathData, filePathModelParams, loadSwitchValues=False):
    ...
    if loadSwitchValues:
        return modelParams, (times, positions, orientations), switchValues
    return modelParams, (times, positions, orientations)
